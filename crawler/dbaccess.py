...
class DBAccess:
    ...
    def setup(self):
        ...
        cursor.execute("CREATE TABLE IF NOT EXISTS resources(url TEXT PRIMARY KEY, path TEXT, lastIndex NUM, summary TEXT, description TEXT, status TEXT)")
    ...
    def add_resource(self, url, path, status, summary, description, transaction):
        cursor = self.connection.cursor()
        cursor.execute("INSERT OR REPLACE INTO resources VALUES (?, ?, ?, ?, ?, ?)", (url, path, time.time(), summary, description, status))
