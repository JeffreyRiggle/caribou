...
use crate::{asset_processor::process_asset, dbaccess::DbConfig, errors::ApiError, file_access::{FileAccess, FileAccessProvider}, models::{AssetDetail, AssetResult, PageAssetResult, QueryRequest}};
...
async fn get_page_assets(pool: web::Data<Pool<DbConfig>>, file_access: web::Data<FileAccessProvider>, base64_url: web::Path<String>) -> Result<impl Responder> {
    let url = BASE64_STANDARD.decode(base64_url.as_str().replace("%2F", "/")).unwrap();
    let result = web::block(move || {
        let mut repository = pool.get().expect("Couldn't get connection from pool");
    ...
    Ok(web::Json(PageAssetResult {
        assets: assets
    }))
}

#[get("/api/v1/{base64_url}/details")]
async fn get_page_details(pool: web::Data<Pool<DbConfig>>, file_access: web::Data<FileAccessProvider>, base64_url: web::Path<String>) -> Result<Json<AssetDetail>, ApiError> {
    let url = BASE64_STANDARD.decode(base64_url.as_str().replace("%2F", "/")).unwrap();
    ...
    Ok(web::Json(process_asset(page_details, file_access.get_ref().clone()).await.unwrap()))
}